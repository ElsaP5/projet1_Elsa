...
function remplir_table(tab) {
	...
	let li_td = document.getElementsByTagName("td");

	for (let i =0; i < 9; i++) {
	console.log(li_td);
/*once we get the list - we need to see it and write them in  */
	li_td[i].innerHTML = tab[i];
}
	return;
}
...
function somme_cases(id_cases) {
	...
	for (let i=0; i < id_cases.length; i++) {
		...
		let contenu_td = td.innerHTML;
		let entier_td = parseInt(contenu_td);
...
/*call on other funtions? */
	ids_correspondants();
	afficher_cases();

console.log(somme_cases) 
}
